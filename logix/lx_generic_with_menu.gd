extends "lxnode.gd"

class_name LXGenericWithMenu

var generic_menu = preload("res://nodes_parts/generic_types_menu.tscn")
var generic_slots  = [[],[]] 

var menu_types:Array = Array()
var generic_default_type = "float"
const generic_menu_name:String = "GenericMenu"


signal recheck_connections(node)

func setup_generic_types_list(types:PoolStringArray):
	get_node("GenericMenu").setup_menu(types)

func reconfigure_generic_slots():
	for input_idx in generic_slots[0]:
		var slot:NodeSlot = inputs[input_idx]
		slot.logix_type = generic_class_names[0]
		_set_input_slot(input_idx, slot)
	var slot_offset:int = len(inputs)
	for output_idx in generic_slots[1]:
		var slot:NodeSlot = outputs[output_idx]
		slot.logix_type = generic_class_names[1]
		_set_output_slot(slot_offset + output_idx, slot)

func _cb_selected_generic_type_from_menu(type_name:String):
	printerr("Called ?")
	generic_class_names[0] = type_name
	reconfigure_generic_slots()
	emit_signal("recheck_connections", self)


func complete_dup():
	var n = .complete_dup()
	n.menu_types = menu_types
	n.generic_slots = generic_slots
	n.generic_menu  = generic_menu
	n._connect_menu()
	return n

func _connect_menu():
	var menu = get_node("GenericMenu")
	if menu != null:
		menu.connect("type_defined", self, "_cb_selected_generic_type_from_menu")
	else:
		printerr("Meep ! Menu is null !")

func set_io(new_inputs:Array, new_outputs: Array):
	printerr("[lx_generic_with_menu] Calling set_io from generic")
	for i in range(0,len(new_inputs)):
		if new_inputs[i].generic == 1:
			generic_slots[0].append(i)
	for i in range(0,len(new_outputs)):
		if new_outputs[i].generic == 1:
			generic_slots[1].append(i)
	.set_io(new_inputs, new_outputs)

func add_types_menu():
	var menu = generic_menu.instance()
	menu.name = generic_menu_name
	add_child(menu)
	menu.setup_menu(menu_types, generic_class_names[0])
	menu.connect("type_defined", self, "_cb_selected_generic_type_from_menu")

func get_logix_class_full_name():
	var selected_type:String = get_node(generic_menu_name).get_selected_type_name()
	var actual_csharp_name:String = _get_csharp_class_name_for(selected_type)
	return logix_class_name + "," + actual_csharp_name

func _report_faulty_conf(serialized_node:Dictionary):
	printerr("Configuration was : %s" % [str(to_json(serialized_node))])

func configure_from_serialized(serialized_node:Dictionary) -> bool:
	if not .configure_from_serialized(serialized_node):
		return false

	if not serialized_node.has("generic"):
		printerr("[BUG] Generic node configuration without a 'generic' section")
		_report_faulty_conf(serialized_node)
		return false

	if not serialized_node["generic"] is Dictionary:
		printerr("[BUG] Expected a JSON object for 'generic' !")
		_report_faulty_conf(serialized_node)
		return false

	var serialized_generic_conf:Dictionary = serialized_node["generic"]

	if not serialized_generic_conf.has("menu"):
		printerr(
			"[BUG] Configuring a generic with type definition from menu " +
			"without specifiying a menu !")
		return false

	if not serialized_generic_conf["menu"] is Array:
		printerr("[BUG] generic -> menu MUST be an array !")
		_report_faulty_conf(serialized_node)
		return false

	self.menu_types = serialized_node["generic"]["menu"]
	# FIXME This should be defined in the JSON file !
	if generic_class_names.empty():
		generic_class_names.append(generic_default_type)
	add_types_menu()
	reconfigure_generic_slots()
	return true

func serialize_def() -> Dictionary:
	var generic_configuration:Dictionary = {
		"menu": self.menu_types
	}
	var main_conf:Dictionary = .serialize_def()
	main_conf["generic"] = generic_configuration
	main_conf["type"]    = "GenericWithMenu"
	return main_conf
	
